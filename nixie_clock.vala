/*
 * Nixie Clock Widget (Optimized with Surface Caching & Fixed GLib Timers)
...
public class NixieClockWindow : Gtk.Window {
    ...
    public int window_type_hint_mode = 0; // 0: Auto, 1: Utility, 2: Dock, 3: Normal
    public int window_layer_mode = 0; // 0: Normal, 1: Above, 2: Below
    ...
    public static void log_debug(string fmt, ...) {
        if (debug_mode) {
            var l = va_list();
            stdout.vprintf(fmt, l);
        }
    }

    public NixieClockWindow() {
        log_debug("[DEBUG] Initializing NixieClockWindow...\n");
    ...
    public void apply_window_type_hint() {
        ...
        } else if (window_type_hint_mode == 3) {
    ...
    public void apply_window_layer() {
        if (window_layer_mode == 1) {
            set_keep_above(true);
            set_keep_below(false);
        } else if (window_layer_mode == 2) {
    ...
    public bool load_config(out int out_x, out int out_y) {
        ...
        try {
            if (kf.load_from_file(path, KeyFileFlags.NONE)) {
                ...
                if (kf.has_key("Settings", "window_layer_mode")) window_layer_mode = kf.get_integer("Settings", "window_layer_mode");
    ...
    public void save_config() {
        ...
        try {
            ...
            kf.set_integer("Settings", "window_layer_mode", window_layer_mode);
    ...
    public void stop_timers() {
        log_debug("[DEBUG] Stopping active timers...\n");
        if (colon_timer_id != 0) {
            Source.remove(colon_timer_id);
            colon_timer_id = 0;
        }
        if (number_timer_id != 0) {
            Source.remove(number_timer_id);
            number_timer_id = 0;
        }
        if (clock_timer_id != 0) {
            Source.remove(clock_timer_id);
            clock_timer_id = 0;
        }
    }
    ...
    public void start_timers() {
        ...
        uint interval = show_seconds ? 500 : 1000;
        log_debug("[DEBUG] Starting clock_timer_id with interval: %u ms (show_seconds: %s)\n", 
                  interval, show_seconds ? "true" : "false");
        ...
        clock_timer_id = GLib.Timeout.add(interval, () => {
            ...
            log_debug("[DEBUG] Clock Timer Tick -> Current: %s | Last Drawn: %s\n", 
                      current_time_str, last_drawn_time_str);
        ...
        if (pulse_colon && animation_mode != 2) {
            ...
            log_debug("[DEBUG] Starting colon_timer_id with interval: %d ms\n", interval_ms);
        ...
        if (pulse_numbers && animation_mode != 2) {
            ...
            log_debug("[DEBUG] Starting number_timer_id with interval: %d ms\n", interval_ms);
    ...
    public void restart_timers() {
        log_debug("[DEBUG] Restarting timers...\n");
        start_timers();
    }
    ...
    private void refresh_bg_cache(double w, double h, double start_x, double start_y, double tube_w, double tube_h, double spacing, double colon_w, double am_w, int num_tubes) {
        log_debug("[DEBUG] Refreshing background cache surface (%dx%d)\n", (int)w, (int)h);
    ...
    private bool on_draw(Cairo.Context cr) {
        ...
        log_debug("[DEBUG] on_draw() Executed -> Rendered Time: %s\n", last_drawn_time_str);
...
public class OptionsDialog : Gtk.Dialog {
    ...
    public OptionsDialog(NixieClockWindow parent_window) {
        ...
        var layer_box = new Gtk.Box(Gtk.Orientation.HORIZONTAL, 12);
        layer_box.pack_start(new Gtk.Label("Window Layer:"), false, false, 0);
        var layer_combo = new Gtk.ComboBoxText();
        layer_combo.append_text("Normal");
        layer_combo.append_text("Above (Always on Top)");
        layer_combo.append_text("Below (Desktop Layer)");
        layer_combo.active = clock_window.window_layer_mode;
        layer_combo.set_tooltip_text("Sets window stacking layer order.");

        layer_combo.changed.connect(() => {
            clock_window.window_layer_mode = layer_combo.active;
            clock_window.apply_window_layer();
            clock_window.save_config();
        });
        layer_box.pack_end(layer_combo, false, false, 0);
        tab_display.pack_start(layer_box, false, false, 0);
        ...
        hint_combo.append_text("Normal");
        hint_combo.active = clock_window.window_type_hint_mode;
        hint_combo.set_tooltip_text("Forces a specific window type hint.");
        ...
        hint_combo.changed.connect(() => {
            if (hint_combo.active != clock_window.window_type_hint_mode) {
                ...
                var msg_dialog = new Gtk.MessageDialog(
                    ...
                    "Window Type Hint changed. Restart clock now to apply?"
                );
        ...
        scale_box.pack_start(new Gtk.Label("Clock Scale:"), false, false, 0);
        ...
        var chk_24h = new Gtk.CheckButton.with_label("Use 24-Hour Format");
        ...
        ampm_box.pack_start(new Gtk.Label("AM/PM Alignment:"), false, false, 0);
        ...
        colon_speed_box.pack_start(new Gtk.Label("Pulse Speed (ms):"), false, false, 0);
        ...
        num_speed_box.pack_start(new Gtk.Label("Pulse Speed (ms):"), false, false, 0);
...
public int main(string[] args) {
    ...
    const OptionEntry[] options = {
        { "debug", 'd', 0, OptionArg.NONE, ref NixieClockWindow.debug_mode, "Enable verbose debug output to stdout", null },
        { null }
    };

    try {
        var opt_context = new OptionContext("- Nixie Tube Desktop Clock");
